 ...
 * Error codes
 */
static const s_CudaErrorStr sCudaDrvErrorString[] =
{
  /**
...
inline void __cuCheckCallSync(CUresult err, const char *call, const char *file, const int line)
{
  if (CUDA_SUCCESS != err) 
  {
    fprintf(stderr, "%s Driver API error = %04d from file <%s>, line %i.\n", call, err, file, line);
    exit(-1);
  }
  if (CUDA_SUCCESS != cuCtxSynchronize()) 
  {
    fprintf(stderr, "%s Driver API error at synchronization = %04d from file <%s>, line %i.\n", call, err, file, line);
    exit(-1);
  }
}


inline void __cuCheckCallNoSync(CUresult err, const char* call, const char *file, const int line)
{
  if (CUDA_SUCCESS != err) 
  {
    fprintf(stderr, "%s Driver API error = %04d from file <%s>, line %i.\n", call, err, file, line);
...
#define cutilDrvCheckCallSync(call)       do { CUresult err = call; __cuCheckCallSync (err, #call, __FILE__, __LINE__); } while (0)
#define cutilDrvCheckCallNoSync(call)     do { CUresult err = call; __cuCheckCallNoSync (err, #call, __FILE__, __LINE__); } while (0)
